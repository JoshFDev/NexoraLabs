...
const garantizarLogrosDefinidos = async () => {
  await Promise.all(
    LOGROS_PREDETERMINADOS.map((def) => Logro.updateOne({ clave: def.clave }, { $setOnInsert: def }, { upsert: true }))
  );
};

const cumplimientosUsuario = async (usuarioId) => {
  const [totalProyectos, totalPostulaciones, totalEquipos, totalComentarios, totalRecursos, usuario] =
    await Promise.all([
      ...
      Usuario.findById(
        usuarioId,
        "apellido_materno pais provincia acerca_de_mi especialidad_principal intereses idiomas educacion"
      ).lean()
    ]);
