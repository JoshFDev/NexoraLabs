...
export const obtenerStats = async (req, res) => {
  try {
    ...
    const [totalUsuarios, totalProyectos, totalEquipos, totalPostulaciones, totalHabilidades, totalRecursos] =
      await Promise.all([
...
export const statsAdmin = async (req, res) => {
  try {
    ...
    const [recursosPorTipo, habilidadesMasPedidas, usuariosRecientes, proyectosRecientes, equiposRecientes] =
      await Promise.all([
        RecursoAprendizaje.aggregate([
