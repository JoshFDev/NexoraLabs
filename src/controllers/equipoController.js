...
export const listarEquipos = async (req, res) => {
  try {
    ...
    if (buscar) {
      filtros.$or = [{ nombre: { $regex: buscar, $options: "i" } }, { descripcion: { $regex: buscar, $options: "i" } }];
    }
...
export const obtenerEquipo = async (req, res) => {
  try {
    const equipo = await Equipo.findById(req.params.id).populate("proyecto_id", "titulo creador_id");
  ...
  try {
    ...
    if (!proyecto_id || !nombre) {
      return res
        ...
        .json(badRequest("Todos los campos obligatorios son requeridos (proyecto_id, nombre)"));
    }
