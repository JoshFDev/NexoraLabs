...
export const misHabilidades = async (req, res) => {
  try {
    const registros = await UsuarioHabilidad.find({ usuario_id: req.usuario.id }).populate(
      "habilidad_id",
      "nombre categoria"
    );
    res.json(registros);
...
export const habilidadesPublicas = async (req, res) => {
  try {
    const registros = await UsuarioHabilidad.find({ usuario_id: req.params.id }).populate(
      "habilidad_id",
      "nombre categoria"
    );
    res.json(registros);
...
export const obtenerUsuarioHabilidad = async (req, res) => {
  try {
    const usuarioHabilidad = await UsuarioHabilidad.findById(req.params.id)
      ...
      .populate("habilidad_id", "nombre");
    if (!usuarioHabilidad)
      return res.status(httpStatus.NOT_FOUND).json(notFound("Relación usuario-habilidad no encontrada"));
