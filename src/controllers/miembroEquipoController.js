...
export const cambiarRolMiembro = async (req, res) => {
  try {
    ...
    const miembroConDatos = await MiembroEquipo.findById(miembro._id).populate(
      "usuario_id",
...
export const solicitarIngreso = async (req, res) => {
  try {
    ...
    const yaPendiente = await SolicitudEquipo.findOne({
      equipo_id: equipoId,
...
export const misSolicitudesEnviadas = async (req, res) => {
  try {
    const solicitudes = await SolicitudEquipo.find({ usuario_id: req.usuario.id }).populate(
      "equipo_id",
      "nombre estado"
    );
...
export const cambiarEstadoSolicitud = async (req, res) => {
  try {
    ...
    const solicitud = await SolicitudEquipo.findById(req.params.id).populate("equipo_id", "estado");
    ...
    await registrarNotificacion({
      ...
      mensaje:
        estado === "aprobada"
          ...
          : `Tu solicitud para unirte al equipo "${solicitudConDatos?.equipo_id?.nombre || ""}" no fue aprobada.`,
      enlace: `/equipos`,
      email: "aceptaciones"
    });
