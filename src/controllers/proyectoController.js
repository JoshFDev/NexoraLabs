  ...
  try {
    ...
    if (!creador_id || !titulo || !descripcion) {
      return res
        ...
        .json(badRequest("Todos los campos obligatorios son requeridos (creador_id, titulo, descripcion)"));
    }
...
//ej: /proyectos?categoria=web&buscar=react&habilidad=id&pagina=1&limite=10&orden=recientes
export const listarProyectos = async (req, res) => {
  try {
    const { categoria, estado, nivel, buscar, creador, habilidad } = req.query;
    ...
    if (buscar) {
      filtros.$or = [{ titulo: { $regex: buscar, $options: "i" } }, { descripcion: { $regex: buscar, $options: "i" } }];
    }
