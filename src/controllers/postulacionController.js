...
export const retirarMiPostulacion = async (req, res) => {
  try {
    const postulacion = await Postulacion.findOne({ _id: req.params.id, usuario_id: req.usuario.id }).populate(
      "proyecto_id",
      "creador_id titulo"
    );
...
export const editarMiPostulacion = async (req, res) => {
  try {
    ...
    if (req.body.habilidades_ofrecidas !== undefined)
      postulacion.habilidades_ofrecidas = req.body.habilidades_ofrecidas;
    await postulacion.save();
