...
export const calificarRecurso = async (req, res) => {
  try {
    ...
    const indice = recurso.comentarios.findIndex((c) => c.usuario_id && String(c.usuario_id) === String(usuarioId));
    ...
    const conDatos = await RecursoAprendizaje.findById(recursoId).populate(
      "comentarios.usuario_id",
      "nombre apellido_paterno rol"
    );
    res.json(conDatos);
  } catch (error) {
    console.log(error);
