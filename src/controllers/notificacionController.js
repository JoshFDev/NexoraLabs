...
export const misNotificaciones = async (req, res) => {
  try {
    ...
    const notificaciones = await Notificacion.find({ usuario_id: req.usuario.id }).sort({ fecha: -1 }).limit(limite);
...
export const marcarTodasLeidas = async (req, res) => {
  try {
    await Notificacion.updateMany({ usuario_id: req.usuario.id, leida: false }, { $set: { leida: true } });
