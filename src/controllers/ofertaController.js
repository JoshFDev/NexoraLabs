...
export const crearOferta = async (req, res) => {
  try {
    ...
    if (!titulo || (titulo || "").trim().length < 5)
      return res.status(httpStatus.BAD_REQUEST).json(badRequest("Escribe un título de al menos 5 caracteres"));
    if (!descripcion || (descripcion || "").trim().length < 20)
      return res.status(httpStatus.BAD_REQUEST).json(badRequest("Escribe una descripción de al menos 20 caracteres"));
...
export const postularAOferta = async (req, res) => {
  try {
    ...
    if (oferta.estado !== "abierta")
      return res.status(httpStatus.CONFLICT).json(conflict("Esta oferta ya no acepta postulaciones"));
    ...
    const existePendiente = await PostulacionOferta.findOne({
      oferta_id: oferta._id,
      usuario_id: req.usuario.id,
      estado: "pendiente"
    });
    if (existePendiente)
      return res.status(httpStatus.CONFLICT).json(conflict("Ya tienes una postulación pendiente en esta oferta"));
...
export const retirarMiPostulacionOferta = async (req, res) => {
  try {
    ...
    if (String(postulacion.usuario_id) !== String(req.usuario.id))
      return res.status(httpStatus.FORBIDDEN).json(forbidden());
    if (postulacion.estado !== "pendiente")
      return res.status(httpStatus.CONFLICT).json(conflict("Solo puedes retirar una postulación pendiente"));
...
export const cambiarEstadoPostulacionOferta = async (req, res) => {
  try {
    ...
    if (postulacion.estado !== "pendiente")
      return res.status(httpStatus.CONFLICT).json(conflict("Solo puedes responder postulaciones pendientes"));
