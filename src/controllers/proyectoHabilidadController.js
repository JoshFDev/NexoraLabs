...
//GET /proyectos-habilidades → listar las relaciones proyecto-habilidad
export const listarProyectoHabilidades = async (req, res) => {
  try {
...
//GET /proyecto-habilidad/:id → ver una relación por id
export const obtenerProyectoHabilidad = async (req, res) => {
  try {
    const proyectoHabilidad = await ProyectoHabilidad.findById(req.params.id)
      .populate("proyecto_id", "titulo")
      .populate("habilidad_id", "nombre");
    if (!proyectoHabilidad)
      return res.status(httpStatus.NOT_FOUND).json(notFound("Relación proyecto-habilidad no encontrada"));
...
//POST /proyecto-habilidad/agregar → crear una relación
export const crearProyectoHabilidad = async (req, res) => {
  try {
...
//PUT /proyecto-habilidad/:id → actualizar una relación
export const actualizarProyectoHabilidad = async (req, res) => {
  try {
...
//DELETE /proyecto-habilidad/:id → eliminar una relación
export const eliminarProyectoHabilidad = async (req, res) => {
  try {
