...
describe("Proyectos", () => {
  test("Crear un proyecto con token y rol válido responde 201", async () => {
    ...
    const res = await request(app).post("/proyecto/agregar").set("Authorization", token).send({
...
const obtenerTokenDe = async (datos) => {
  let res = await request(app).post("/usuario/login").send({ email: datos.email, password: datos.password });
  ...
  if (res.status !== 200) {
    ...
    res = await request(app).post("/usuario/login").send({ email: datos.email, password: datos.password });
  }
...
describe("Recursos de aprendizaje", () => {
  test("Crear un recurso sin token responde 401", async () => {
    const res = await request(app).post("/recurso-aprendizaje/agregar").send({ titulo: "Curso sin token" });
  ...
  test("Un mentor crea un recurso y responde 201", async () => {
    ...
    const res = await request(app).post("/recurso-aprendizaje/agregar").set("Authorization", token).send({
...
describe("Ofertas de empleo", () => {
  ...
  test("Un mentor crea una oferta y responde 201", async () => {
    const token = await obtenerTokenDe(usuarioMentor);
    const res = await request(app).post("/oferta/agregar").set("Authorization", token).send({
  ...
  test("Un desarrollador puede postularse a una oferta ajena", async () => {
    const tokenMentor = await obtenerTokenDe(usuarioMentor);
    const oferta = await request(app).post("/oferta/agregar").set("Authorization", tokenMentor).send({
...
describe("Equipos", () => {
  ...
  test("Crear un equipo sin token responde 401", async () => {
    const res = await request(app).post("/equipo/agregar").send({ nombre: "Equipo sin token" });
...
describe("Eliminación de cuenta", () => {
  ...
  test("Confirmar eliminación con código correcto elimina la cuenta", async () => {
    ...
    const solicitud = await request(app).post("/usuario/eliminar/solicitar").set("Authorization", token);
  ...
  test("Confirmar sin código responde 400", async () => {
    ...
    const res = await request(app).post("/usuario/eliminar/confirmar").set("Authorization", token).send({});
...
describe("Recuperación de contraseña", () => {
  ...
  test("Solicitar recuperación de un correo inexistente responde 200 sin código", async () => {
    const res = await request(app).post("/usuario/recuperar/solicitar").send({ email: "nadie@testing.com" });
  ...
  test("Cambiar la contraseña con el código y poder iniciar sesión con la nueva", async () => {
    ...
    const solicitud = await request(app).post("/usuario/recuperar/solicitar").send({ email: datosRecuperable.email });
    expect(solicitud.status).toBe(200);
...
describe("Preferencias de notificación e intereses", () => {
  ...
  test("Crear un proyecto crea una notificación para quien tenga intereses que coincidan", async () => {
    ...
    const proyecto = await request(app).post("/proyecto/agregar").set("Authorization", tokenCreador).send({
...
describe("Notificaciones", () => {
  ...
  test("Marcar todas como leídas, verificar el contador y eliminar una notificación", async () => {
    ...
    const marcadas = await request(app).post("/notificaciones/marcar-todas").set("Authorization", token);
    ...
    const borrar = await request(app).delete(`/notificacion/${notif._id}`).set("Authorization", token);
...
describe("Comentarios de proyectos", () => {
  ...
  test("Crear comentarios requiere token y un contenido no vacío", async () => {
    ...
    const proyecto = await request(app).post("/proyecto/agregar").set("Authorization", tokenAutor).send({
      creador_id: perfil.body._id,
    ...
    const sinToken = await request(app).post(`/proyecto/${proyecto.body._id}/comentar`).send({ contenido: "Hola" });
  ...
  test("El autor puede eliminar su comentario, un ajeno no", async () => {
    ...
    const prohibido = await request(app).delete(`/comentario/${crear.body._id}`).set("Authorization", tokenAjeno);
    expect(prohibido.status).toBe(403);

    const borrar = await request(app).delete(`/comentario/${crear.body._id}`).set("Authorization", tokenAutor);
    expect(borrar.status).toBe(200);
  });
});
